...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('sentence-search').addEventListener('input', function(e) {
...
async function loadSentences() {
    try {
        ...
        document.getElementById('sentence-list').innerHTML = '<p class="error">Error loading sentences</p>';
    }
}
...
function renderSentenceList(sentences) {
    ...
    listEl.innerHTML = sentences.map(id => 
...
async function loadSentence(sentenceId) {
    ...
    // Update UI
    document.getElementById('current-sentence-id').textContent = `Sentence ID: ${sentenceId}`;
    ...
    // Show loading
    document.getElementById('visualizations').innerHTML = '<div class="loading">Loading sentence data...</div>';
    ...
    try {
        ...
        document.getElementById('visualizations').innerHTML = 
            `<div class="error">Error loading sentence: ${error.message}</div>`;
    }
}
...
function renderMetricsTable() {
    ...
    // Gold reference
    rows.push(createMetricsRow('Gold (Reference)', {
        AER: 0,
        precision: 1,
        recall: 1,
        f1: 1
...
function renderVisualizations() {
    ...
    const goldHtml = createVisualizationPanel('Gold Alignment', sentenceData.gold, null, sentenceData.gold, true);
    
    // System visualizations
    const systemsHtml = Object.keys(sentenceData.systems).map(sysKey => {
        const sys = sentenceData.systems[sysKey];
...
function createVisualizationPanel(title, systemData, predictedPairs, goldData, isGold) {
    ...
    const arWords = arTokens.map((token, idx) => 
        `<span class="word-box" data-ar-index="${idx}" data-index="${idx + 1}" data-word="${token}">${token}</span>`
    ).join('');
    
    const enWords = enTokens.map((token, idx) => 
        `<span class="word-box" data-en-index="${idx}" data-index="${idx + 1}" data-word="${token}">${token}</span>`
    ).join('');
    
    return `
        <div class="visualization-panel" data-system="${isGold ? 'gold' : systemData.display_name}">
            <h3>${title}</h3>
            <div class="alignment-container" id="alignment-${isGold ? 'gold' : systemData.display_name.replace(/\s+/g, '-')}">
                <div class="sentence-row" id="ar-row-${isGold ? 'gold' : systemData.display_name.replace(/\s+/g, '-')}">
                    <span class="sentence-label">Arabic:</span>
                    <div class="words-container">
                        ${arWords}
                    </div>
                </div>
                <div class="sentence-row" id="en-row-${isGold ? 'gold' : systemData.display_name.replace(/\s+/g, '-')}">
                    <span class="sentence-label">English:</span>
                    <div class="words-container">
                        ${enWords}
                    </div>
                </div>
                <svg class="alignment-svg" id="svg-${isGold ? 'gold' : systemData.display_name.replace(/\s+/g, '-')}"></svg>
            </div>
        </div>
    `;
}
...
function renderAllAlignments() {
    ...
    renderAlignmentLines('gold', sentenceData.gold, goldAlignments, goldAlignments, true);
    ...
    Object.keys(sentenceData.systems).forEach(sysKey => {
        ...
        renderAlignmentLines(panelId, sys, sys.predicted_alignments || [], goldAlignments, false);
    });
}

// Render alignment lines for a panel
function renderAlignmentLines(panelId, systemData, predictedAlignments, goldAlignments, isGold) {
